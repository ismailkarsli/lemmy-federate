...
<template>
	...
	<v-container v-if="instance">
			...
			<v-row>
				<v-col cols="12">
					<v-row>
						...
						<v-checkbox
							v-if="!isGeneric"
							label="Auto add communities"
							v-model="instance.auto_add"
							hide-details
						/>
						<v-checkbox
							v-if="!isGeneric"
							label="Cross software"
							v-model="instance.cross_software"
							hide-details
						/>
					</v-row>
					<v-alert
						v-if="showHints && !isGeneric"
						type="info"
						color="primary"
						variant="text"
						density="compact"
					...
					>
						<p>
							<strong>Auto add communities</strong>: Lemmy Federate will
							regularly check your instance and add any communities it finds
							automatically.
						</p>
						<p>
							<strong>Cross software</strong>: Allow the bot to federate with
							instances that use other types of Fediverse software than yours.
						</p>
						<p>
							For example, as a
							{{ getHumanReadableSoftwareName(instance.software) }}
							instance, tick this option to follow instances other than
							{{ getHumanReadableSoftwareName(instance.software) }}
							such as
				...
				<v-col v-if="!isGeneric" cols="12">
					...
					>
						<p>
							...
							Your instance will both follow and be followed by other instances.
						</p>
						<p>
							<strong>Seed only:</strong>
							Your instance will not follow other instances' communities, but
							others can follow yours.
						</p>
					</v-alert>
				</v-col>
              ...
              "
				...
				<v-col cols="12">
					...
					<v-alert
						...
						density="compact"
					>
						<strong>Reset subscriptions</strong>: The bot will unsubscribe from
						all communities. This can be useful when you want to disable the
						tool. If the tool is enabled, then it will re-subscribe later.
					</v-alert>
				</v-col>
			</v-row>
			<v-snackbar
		...
		<v-row style="min-height: 16em">
			<v-col cols="12" lg="6">
				<v-app-bar-title class="mb-2">Allowed instances </v-app-bar-title>
				<v-alert
					v-if="showHints"
					type="info"
					color="primary"
					...
					density="compact"
				>
					This list is explicit: if you add at least one instance, all others
					will be blocked.
				</v-alert>
				<v-autocomplete
					label="instance to allow..."
			...
			<v-col cols="12" lg="6">
				<v-app-bar-title class="mb-2">Blocked instances </v-app-bar-title>
				<v-alert
					v-if="showHints"
					type="info"
					color="primary"
					...
					density="compact"
				>
					Unlike the allow list, you will continue to follow instances other
					than those you block.
				</v-alert>
				<v-autocomplete
					label="instance to block..."
					v-model="blockedInstance"
					:items="filteredAllInstances"
					item-title="host"
					item-value="id"
				/>
