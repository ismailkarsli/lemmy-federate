...
<template>
	<v-layout class="rounded rounded-md d-flex flex-column fill-height">
		<v-app-bar title="Lemmy Federate">
			<template v-slot:append>
				...
				<v-btn to="/manage" class="mx-1">
					...
					<span v-else-if="authStore.authenticated"
						>{{ `Manage ${authStore.instance?.host}` }}</span
					>
		...
		<v-footer
			class="flex-grow-0 mb-2 bg-transparent text-no-wrap d-flex flex-column flex-sm-row"
			style="margin: 0 auto; width: 100%; max-width: 1200px;"
		>
			<div>
				<v-btn
				...
				<v-btn
					href="https://lemmyverse.link/c/lemmyfederate@lemy.lol"
					variant="text"
					class="text-none"
					target="_blank"
				>
					<img
						src="./assets/lemmy.svg"
						class="v-icon v-icon--size-x-large mr-2"
						alt="Lemmy"
					>
					Subscribe on Lemmy
				</v-btn>
			...
			<v-divider></v-divider>
			<div style="padding-right: 16px;"><strong>Lemmy Federate</strong></div>
		</v-footer>
	</v-layout>
</template>
